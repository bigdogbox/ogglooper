#!/usr/bin/env python3
#
# This file is part of CrossLooper, bundled with OggLooper.
# Copyright (C) 2023-2024 Splendide Imaginarius.
# CrossLooper is a heavily modified fork of syncstart, (C) 2021 Roland Puntaier.
# CrossLooper is free software under the GNU General Public License, version 3
# or (at your option) any later version. See the LICENSE file for the full text.
# Upstream: https://github.com/Splendide-Imaginarius/crosslooper
#
# MODIFICATIONS made for OggLooper (per GPLv3 section 5a):
#   2026-06-16 (bigdogbox): Changed the matplotlib backend from 'TkAgg' to 'Agg'.
#       OggLooper never displays CrossLooper's diagrams (show=False), and Agg
#       avoids requiring Tk / a display when run as a subprocess.
#   2026-06-16 (bigdogbox): Added "best_length = 0" initialization before the loop
#       search to avoid a NameError on very short inputs where no candidate
#       is found.
#
import matplotlib
...
# OggLooper: use the headless Agg backend. This GUI never shows CrossLooper's
# diagrams (show=False), so Agg avoids needing Tk/a display in the subprocess.
matplotlib.use('Agg')
